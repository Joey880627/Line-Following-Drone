...
def pixel_to_cm(pixel, distance):
    # adjust feedbacks
    sensitivity = 10
    distance = 60 + (distance - g.constant_height) / sensitivity
...
def mission(image, mission_name, drone_height):
    ...
    if (mission_name == 'takeoff') or (mission_name == 'takeoff_loiter'):
    ...
    elif (mission_name == 'lf_1') or (mission_name == 'lf_2') or (mission_name == 'lf_3') or (mission_name == 'lf_4'):
        feedbacks['roll'], _, img1 = f.roll_pitch(image, use_dilate=True)
        if (mission_name == 'lf_1') or (mission_name == 'lf_2'):
            feedbacks['yaw'], img2 = f.yaw(image)
        else:
        ...
        feedbacks['roll'], feedbacks['pitch'], img1 = f.roll_pitch(image, use_dilate=True, method='red')
        img2 = image
    elif mission_name == 'l_color':
        feedbacks['roll'], feedbacks['pitch'], img1 = f.roll_pitch(image, use_dilate=True, method=flags['color'])
        img2 = image
    ...
    if mission_name == 'l_start' or mission_name == 'l_end':
        feedbacks['throttle'] = drone_height - (-200)
    elif mission_name == 'l_red' or mission_name == 'l_color':
...
if __name__ == '__main__':
    ...
    mission_names = ['takeoff', 'takeoff_loiter', 'lf_1', 'led_loiter',
                     'lf_2', 'lf_3', 'st_blue', 'st_red', 'st_green',
                     'lf_4', 'l_red', 'l_color', 'l_start', 'l_end']
    try:
    ...
    try:
        ...
        color_mission = None
    flags = {'mission': 'takeoff', 'color': color_mission, 'loiter_stability': 0, 'lf_startTime': 0}
    loiter_stable_time = 25
    loiter_stability_range = dict({'throttle':40, 'pitch':g.exe_time*2//3, 'roll':g.exe_time*2//3, 'yaw':10})
    ...
    while True:
        ...
        # Loiter on LED
            ...
            if flags['loiter_stability'] > loiter_stable_time:
                flags['mission'] = 'lf_2'
                flags['lf_startTime'] = time.time()
                flags['loiter_stability'] = 0
        elif flags['mission'] == 'lf_2':
            if image_color == None:
                flags['mission'] = 'lf_3'
        elif flags['mission'] == 'lf_3':
            lf_3_time = time.time() - flags['lf_startTime']
            if (image_color == 'blue') and (lf_3_time >= 10):
                flags['mission'] = 'st_blue'
                flags['lf_startTime'] = 0
            ...
            if flags['loiter_stability'] > loiter_stable_time:
                ...
                if flags['color'] == 'green':
                    ...
                    f.sandbag(False)
                flags['loiter_stability'] = 0
        elif flags['mission'] == 'lf_4':
            lf_4_time = time.time() - flags['lf_startTime']
            if image_color != 'green':
                f.sandbag(False)
            if (image_color == 'red' or image_color == 'green') and (lf_4_time >= 10):
                flags['mission'] = 'l_red'
            ...
            if (abs(feedbacks['throttle'])<loiter_stability_range['throttle'])&(abs(feedbacks['pitch'])<loiter_stability_range['pitch']):
                ...
                flags['loiter_stability'] = 0
            if flags['loiter_stability'] > loiter_stable_time:
                flags['mission'] = 'l_start'
                flags['loiter_stability'] = 0
        elif flags['mission'] == 'l_start':
            if drone_height < 20:
                flags['mission'] = 'l_end'
